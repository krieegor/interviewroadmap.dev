...
export async function generateStaticParams({ params }: { params: { locale: string } }) {
  ...
  const terms = await getAllTerms(params.locale);
...
export async function generateMetadata({
  ...
  const term = await getTermBySlug(slug, rawLocale);
  if (!term) return {};
  return {
...
export default async function GlossaryTermPage({
  ...
  const allTerms = await getAllTerms(locale);
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      <div className="mt-4">
        <CopyLinkButton path={`/${locale}/glossario/${frontmatter.slug}`} dict={dict} />
      </div>
              ...
              <ul className="mt-2 flex flex-wrap gap-2">
                {relatedTerms.map((relatedSlug) => {
                  ...
                  return (
                    <li key={relatedSlug}>
                      <Link
                        href={`/${locale}/glossario/${relatedSlug}`}
      ...
      {previous || next ? (
        ...
        >
          <div>
            {previous ? (
              <Link
                href={`/${locale}/glossario/${previous.slug}`}
          ...
          <div>
            {next ? (
              <Link
                href={`/${locale}/glossario/${next.slug}`}
