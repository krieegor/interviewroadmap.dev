...
export async function generateMetadata({
  ...
  return {
    ...
    description: formatTemplate(dict.glossarioIndex.description, { siteName: siteConfig.name }),
  };
}
...
export default async function GlossarioPage({
  ...
  const terms = await getAllTermsFull(locale);
        ...
        {terms.map(({ frontmatter, Content }) => (
          <section key={frontmatter.slug} id={frontmatter.slug} className="scroll-mt-20 py-6">
            <h2 className="text-xl font-semibold text-[var(--color-text)]">
              <Link
                href={`/${locale}/glossario/${frontmatter.slug}`}
