...
- **`/[locale]` (seletor de trilha) e `/[locale]/[tech]` (home de cada trilha)**: a vitrine do produto,
  ...
  - **Scroll da página nunca controla conteúdo 3D**: feedback direto do autor após a primeira versão
    (que ligava a câmera ao scroll), que incomodava ao rolar a página e ver a cena mudar sozinha. O hero do
    Kafka (`src/components/hero/KafkaHero.tsx`) hoje é autocontido: autoplay por `setInterval` avança a etapa
