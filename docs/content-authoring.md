...
Um arquivo de conteúdo pertence a exatamente um locale, com `mesmo nome de arquivo` nos dois idiomas quando o
...
4. `relatedChapters` deve apontar apenas para slugs de capítulos que já existem, pois o script de validação
...
novo, escreva a versão em português primeiro e traduza para `en/` seguindo os passos abaixo. Manter os
...
4. `<ExemploFinanceiro>` mantém o contexto financeiro brasileiro (PIX, boleto, valores em BRL) mesmo na
   versão em inglês, pois é conteúdo real do domínio do livro; não precisa ser adaptado para outro mercado, só
   traduzido.
